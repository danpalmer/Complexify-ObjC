...
static NSArray *charsetsArray = nil;
...
+ (void)checkComplexityOfPassword:(NSString *)password completionHandler:(void (^)(BOOL, CGFloat))completionHandler {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSString* str in commonPasswordsArray) {
            if ([[password lowercaseString] isEqualToString:str]) {
                inBanlist = YES;
                break;
            }
        }
        ...
        if (!inBanlist) {
            for (NSInteger i = charsetsArray.count - 1; i >= 0; i--) {
